...
class IdentityProviderAPI:
    ...
    async def get_users(self, page: int, size: int, access_token: str) -> UsersPagination:
        ...
        async with AsyncClient() as client:
            func: Awaitable = client.get(f'http://158.160.164.167/api/identity-provider/users', headers=headers, params=params)
    ...
    async def get_user(self, user_id: UUID, access_token: str) -> UserModel:
        ...
        async with AsyncClient() as client:
            func = client.get(f'http://158.160.164.167/api/identity-provider/users/{user_id}', headers=headers)
            response: Response | None = await self._circuit_breaker.request(func)
    ...
    async def get_me(self, access_token: str) -> UserModel:
        ...
        async with AsyncClient() as client:
            func = client.get(f'http://158.160.164.167/api/identity-provider/users/me', headers=headers)
            response: Response | None = await self._circuit_breaker.request(func)
    ...
    async def create_user(self, user_input: InputUser) -> UserModel:
        ...
        async with AsyncClient() as client:
            func = client.post(f'http://158.160.164.167/api/identity-provider/users', json=body)
            response: Response | None = await self._circuit_breaker.request(func)
    ...
    async def delete_user(self, user_id: UUID, access_token: str) -> None:
        ...
        async with AsyncClient() as client:
            func = client.delete(f'http://158.160.164.167/api/identity-provider/users/{user_id}', headers=headers)
            response: Response | None = await self._circuit_breaker.request(func)
    ...
    async def update_user(self, user_id: UUID, user_update: UpdateUser, access_token: str) -> UserModel:
        ...
        async with AsyncClient() as client:
            func = client.put(f'http://158.160.164.167/api/identity-provider/users/{user_id}', headers=headers, json=body)
            response: Response | None = await self._circuit_breaker.request(func)
