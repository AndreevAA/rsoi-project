...
class NotesServiceAPI:
    ...
    async def get_notes(self, namespace_id: UUID, page: int, size: int, access_token: str) -> NotesPagination:
        ...
        async with AsyncClient() as client:
            func: Awaitable = client.get(f'http://158.160.164.167/api/notes-service/notes', headers=headers, params=params)  # type: ignore
    ...
    async def get_note(self, note_id: UUID, access_token: str) -> NoteModel:
        ...
        async with AsyncClient() as client:
            func = client.get(f'http://158.160.164.167/api/notes-service/notes/{note_id}', headers=headers)
            response: Response | None = await self._circuit_breaker.request(func)
    ...
    async def create_note(self, note_input: InputNote, access_token: str) -> NoteModel:
        ...
        async with AsyncClient() as client:
            func = client.post(f'http://158.160.164.167/api/notes-service/notes', headers=headers, json=body)
            response: Response | None = await self._circuit_breaker.request(func)
    ...
    async def update_note(self, note_update: UpdateNote, access_token: str) -> NoteModel:
        ...
        async with AsyncClient() as client:
            func = client.put(f'http://158.160.164.167/api/notes-service/notes', headers=headers, json=body)
            response: Response | None = await self._circuit_breaker.request(func)
    ...
    async def delete_note(self, note_id: UUID, access_token: str) -> None:
        ...
        async with AsyncClient() as client:
            func = client.delete(f'http://158.160.164.167/api/notes-service/notes/{note_id}', headers=headers)
            response: Response | None = await self._circuit_breaker.request(func)
